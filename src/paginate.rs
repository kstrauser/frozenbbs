...
/// We're every going to generate lots of long runs of newlines in the first place. Second, our
/// pages are only ever going to be 200 bytes long at most, so this can't ever grown to be
/// O(1_000_000_000**2) or anything like that. I'd vastly rather optimize for simplicity than
/// sophistication here.
fn shrink(text: String) -> String {
    // Pages never have leading whitespace.
    let mut text = text.trim_start().to_string();
    // Lines within a page have have a blank line between them. 2 newlines in a row are fine.
    ...
    // Client apps generate collapse runs of spaces into single spaces. Don't waste bandwidth
    // on those.
    while text.contains("  ") {
        text = text.replace("  ", " ");
    }
...
pub fn splitted(text: String, max_length: usize) -> Vec<String> {
    ...
    // Text with already be shrunk and trimmed before it gets here, so don't bother with doing it
    // again.
...
/// Do whatever we reasonably can to page a series of lines into pages not longer than the
...
pub fn paginate(lines: Vec<String>, max_length: usize) -> Vec<String> {
    ...
    for line in lines {
        ...
        if buf.len() + 1 + line.len() > page_length {
            ...
            // Start a new page with the line.
            buf = line.clone();
            continue;
        }
    ...
    // Does the remaining buffer have anything other than whitespace? Another page!
